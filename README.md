...
<!-- USI 棋譜・最終局面・スライダー付き -->
<shogi-board kif="startpos moves 7g7f 3c3d 2g2f" ply="-1" slider></shogi-board>
```
...
| `ply`      | 整数                  | `0`     | 初期表示手数。`-1` で最終局面                          |
| `slider`   | 属性の有無            | なし    | 再生ボタン・スライダー・手数カウンターを表示          |
...
- **▲ / △ マーカークリック** → 盤を反転（視点切替）
- スライダーをドラッグ → 任意の手数へ
- **▶ / ❚❚ ボタン**（`slider` 指定時）→ 自動再生の開始 / 停止（最終手で自動停止）
...
shogi-board {
  ...
  --ps-frame: rgba(26, 20, 16, 0.18); /* 盤を囲む枠の色 */
}
```
