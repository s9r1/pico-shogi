...
- `src/styles.ts` — Shadow DOM に注入する CSS（`:host` の CSS 変数でカスタマイズ）。既定色は `light-dark()` + `@supports` フォールバックでページの `color-scheme` に追従する。
- `src/types.ts` — 描画用の型（`BoardState` / `BoardCell` / `HandCount` / `Viewpoint`）。tsshogi の `Position` から描画に必要な最小情報を切り出した形。
- `tests/parser.test.ts` — パーサのユニットテスト（vitest / node 環境）。
- `tests/element.test.ts` — Web Component のスモークテスト（vitest / happy-dom 環境）。

依存は将棋ロジックを担う [tsshogi](https://github.com/sunfish-shogi/tsshogi) のみ。ビルドは Vite で `dist/pico-shogi.js`（IIFE・`<script>` 用）と `dist/pico-shogi.mjs`（ESM・`exports.import` 用）を出力し、`tsc -p tsconfig.build.json` で `dist/*.d.ts` を生成する。
